...
class AccountPaymentMakeBeforeBilling(models.TransientModel):
    _name = 'account.payment.make.before.billing'
    _description = "Register Payment Make"
    ...
    line_ids = fields.Many2many('account.move.line', 'account_payment_make_register_move_line_rel', 'wizard_id', 'line_id',
    ...
    @api.depends('amount_payment', 'source_amount', 'source_amount_currency', 'source_currency_id', 'company_id', 'currency_id', 'payment_date')
    def _compute_amount(self):
        for wizard in self:
            ...
            if wizard.amount_payment <= wizard.amount:
                wizard.amount = wizard.amount_payment
    ...
    def _create_payments(self):
        ...
        _logger.info('\nto_reconcileto_reconcile\n %r \n\n', [to_reconcile, amls])
        amls.mapped('move_id').with_context(payment=True).action_post()
